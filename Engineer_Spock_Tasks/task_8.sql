...
ALTER VIEW orders_sub_dates RENAME TO orders_sub_dates_copy;

CREATE OR REPLACE VIEW orders_sub_dates AS
SELECT order_date, required_date, shipped_date, ship_postal_code, ship_country, 
company_name, contact_name, phone, last_name, first_name, title, customers.postal_code, reports_to
FROM orders
JOIN employees USING(employee_id)
JOIN customers USING(customer_id);

SELECT *
FROM orders_sub_dates
ORDER BY ship_country ASC;

DROP VIEW orders_sub_dates_copy;
...
VALUES (
89, 'something', 777, 25, '10 boxes', 18, 125, 25, 15, 1
);